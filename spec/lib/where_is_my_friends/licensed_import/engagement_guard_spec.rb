...
RSpec.describe WhereIsMyFriends::LicensedImport::EngagementGuard do
  context "when seven mature translations have no human reply" do
    fab!(:unanswered_records) do
  ...
  context "when replies arrive after the seven-day window" do
    fab!(:human, :user)
    fab!(:late_reply_records) do
      7.times.map do |index|
    ...
    it "does not count those replies" do
      SiteSetting.licensed_import_enabled = true
      notifier = instance_spy(WhereIsMyFriends::LicensedImport::AdminNotifier)
      late_reply_records
