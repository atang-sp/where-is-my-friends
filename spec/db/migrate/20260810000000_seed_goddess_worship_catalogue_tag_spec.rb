...
RSpec.describe SeedGoddessWorshipCatalogueTag do
  before do
    ...
    Tag.where(name: "女神崇拜").delete_all
  end

  after { ActiveRecord::Migration.verbose = @original_verbose }

  it "adds the curated tag when upgrading an existing site" do
    allow(Migration::Helpers).to receive(:existing_site?).and_return(true)

    described_class.new.up

    expect(Tag.exists?(name: "女神崇拜")).to eq(true)
  end
  ...
  it "leaves fresh-install rows to the plugin fixture" do
    ...
    expect(Tag.exists?(name: "女神崇拜")).to eq(false)
  end
end
