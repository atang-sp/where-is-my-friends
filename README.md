...
每个候选继续服从原子功能边界：兴趣引导关闭时跳过 onboarding，实践邀请关闭时跳过邀请和会话行动，个性化关闭时不调用推荐器，动态关闭或专用分区不安全时跳过动态。同城发现保持城市模式始终可用的现有合同；总开关、新入口开关关闭或用户未登录时 endpoint fail closed，客户端在 `empty` 或请求失败时不留下空白区域。关闭新入口会恢复旧首页模块，便于独立回退。
